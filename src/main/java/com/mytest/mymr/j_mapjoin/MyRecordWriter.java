...
class MyRecordWritert extends RecordWriter<MyKeyBean, MyBean> {
    private FSDataOutputStream nomalOut;
    private FSDataOutputStream otherOut;

    public MyRecordWritert(TaskAttemptContext job) throws IOException {
        FileSystem fs = FileSystem.get(job.getConfiguration());
        nomalOut = fs.create(new Path("src/output/map_join/out2/nomal.txt"));
        otherOut = fs.create(new Path("src/output/map_join/out2/orther.txt"));
    }

    @Override
    public void write(MyKeyBean key, MyBean value) throws IOException, InterruptedException {
        String preNum = key.getPhoneNum().substring(0, 3);
