...
//        job.setNumReduceTasks(5); // 此时无法设置RT
        ...
        // 加入缓存
        job.addCacheFile(new URI("src/source/map_join2/map_join_total.txt"));
        // 6 设置输入和输出路径
        FileInputFormat.setInputPaths(job, new Path("src/source/map_join2/datas"));
        FileOutputFormat.setOutputPath(job, new Path("src/output/map_join2/out1"));
        // 7 提交job
