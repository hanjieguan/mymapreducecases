...
class MyRecordWritert extends RecordWriter<Text, MyBean> {
    ...
    public MyRecordWritert(TaskAttemptContext job) throws IOException {
        ...
        nomalOut = fs.create(new Path("src/output/outputformat/out2/nomal.txt"));
        otherOut = fs.create(new Path("src/output/outputformat/out2/orther.txt"));
    }
...
//            System.out.println(preNum);
            nomalOut.writeBytes(key.toString()+" "+value+"\n");
        }else {
            otherOut.writeBytes(key.toString()+" "+value+"\n");
        }
    }
